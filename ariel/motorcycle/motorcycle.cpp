...
void parseClearance()
{
	...
	while (!inUsers.eof())
	{
		...
		inUsers >> id[x] >> name[x] >> age[x] >> sex[x];
		x++;
	}
...
void parseProducts()
{
	...
	while (!inProducts.eof())
	{
		...
		// store data to variables
		inProducts >> brand[x] >> model[x] >> price[x] >> engine[x] >> transmission[x] >> displacement[x] >> fuel[x];
...
int getOrder()
{
	...
	while (true)
	{
		...
		cout << setw(1) << "#" << setw(9) << "Brand" << setw(10) << "Model" << setw(13) << "Price (PHP)" << setw(34) << "Engine Type" << setw(14) << "Transmission" << setw(14) << "Displacement" << setw(10) << "Fuel" << endl;

		for (int x = 0; x < pc; x++)
			cout << setw(1) << (x + 1) << setw(9) << brand[x] << setw(10) << model[x] << setw(13) << price[x] << setw(34) << engine[x] << setw(14) << transmission[x] << setw(14) << displacement[x] << setw(10) << fuel[x] << endl;
...
void displayReceipt(int userID, int productID, int payM, int payO)
{
	...
	// Columns
	cout << setw(8) << "Brand" << setw(10) << "Model" << setw(13) << "Price (PHP)" << setw(34) << "Engine Type" << setw(14) << "Transmission" << setw(14) << "Displacement" << setw(6) << "Fuel" << setw(25) << "Payment" << endl;

	// Row
	cout << setw(8) << brand[productID] << setw(10) << model[productID] << setw(13) << priceCheck << setw(34) << engine[productID] << setw(14) << transmission[productID] << setw(14) << displacement[productID] << setw(6) << fuel[productID] << setw(25) << paymentM + paymentO << endl;
}
